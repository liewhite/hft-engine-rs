...
### 1.9 就绪状态前置：宁可晚开工，不要让消费者处理"还没准备好"

能在**投产期一次性备齐**的状态，就不要让策略在运行期反复检查"到了没有"。
备不齐就拒绝启动 —— 事务性投产的 `Err` 等于"什么都没发生"，代价有界且可见。

已经这么做的：投产撤残单 → 拉持仓基线 → spawn（**出生即带基线**）→ 注册 → 放行行情。
executor 在注册之前收不到任何事件；作用域内每条腿的持仓在第一条事件之前就已写满
（见 `provisioning.rs` 的 `executor_baselines`）。于是"持仓未查询到"这个状态
对策略不可达 —— 这是 1.1 的阶梯用在**时序**上：让中间状态不存在，
优先于让每个调用点去检查它。

**但这条不是越多越好，两个判据卡住它的边界：**

1. **每种情形都得有真值，否则填不干净。** 持仓可以：有凭证的所用 REST 快照，
   无凭证的所"本引擎持仓恒为 0"是事实。净值不行：无凭证的所压根没有账户，
   净值不是 0 而是**不存在** —— 强行填 0 只是把"未知"换了个伪装。
   前置之后 `Option` 仍在、检查没省掉，就不值得做。

2. **流不能当快照前置。** BBO 是流：预取只能拿到某一刻的一跳，而策略要的是持续新鲜
   的报价 —— 陈旧度检查照样得写，`Option` 消失了检查只是换了形式。
   而且"等所有订阅的 BBO 都来一跳才放行"会让一个不活跃的 symbol 阻塞整批投产。

**代价要如实记账**：每加一道前置检查，重启循环的面就扩大一次。配置错导致的持久失败，
重启循环是对的（能被发现）；交易所 REST 瞬时抖动导致的失败，重启循环会让实例在市场
波动时反复掉线。所以前置检查同样受 1.2 约束 —— **没有区分力的判据不配拥有停机权**。
...
11. 我在统计"实测调用面"吗？**样本覆盖了库外的消费者吗？**只扫本仓库会得出一个
    看着精确、实则漏了真实用户的数字 —— R3 就是这么把 12 个方法数成 9 个的，
    代价是连着两个已发布 tag 的破坏性抖动（v0.15 加 `position()`、v0.16 又删掉）。
